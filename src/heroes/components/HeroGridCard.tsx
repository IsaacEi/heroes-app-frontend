...
export const HeroGridCard: FC<Props> = ({ hero }) => {
  const navigate = useNavigate();
  const { isFavoriteHero, toggleFavorite } = use(FavoriteHeroContext);
  ...
  return (
    <Card className="group overflow-hidden hover:shadow-xl transition-all duration-300 hover:-translate-y-1 bg-linear-gradient-to-br from-white to-gray-50">
          <div className="relative h-64 ">
            ...
            <div className="absolute top-3 left-3 flex items-center gap-2">
              <div className={`w-3 h-3 rounded-full ${hero.status.toLocaleLowerCase() === "active" ? "bg-green-500" : "bg-red-500"}`} />
            ...
            {/* Universe badge */}
            <Badge className={`absolute top-3 right-3 text-xs ${hero.universe.toLocaleLowerCase() === "dc" ? "bg-blue-600" : "bg-red-600"} text-white`}>{hero.universe}</Badge>

            {/* Favorite button */}
            <Button size="sm" variant="ghost" className="absolute bottom-3 right-3 bg-white/90 hover:bg-white"
              onClick={() => toggleFavorite(hero)}
            >
              <Heart className={`${isFavoriteHero(hero) ? "fill-red-500 text-red-500" : "text-gray-400"}`} />
            </Button>
          ...
          <CardHeader className="py-3 z-10 bg-gray-100/50 backdrop-blur-sm relative top-1 group-hover:-top-2.5 transition-all duration-300">
            <div className="flex justify-between items-start">
              ...
              <Badge className={`text-xs ${hero.category.toLocaleLowerCase() === "hero" ? "bg-green-100 text-green-800 border-green-200" : "bg-red-100 text-red-800 border-red-200"}`}>{hero.category}</Badge>
            </div>
